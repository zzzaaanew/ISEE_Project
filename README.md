...
AcmeTrace에서 XID episode 전후의 GPU 텔레메트리 변화를 분석하고, 동일 GPU의 요일·시간대·사전 활동량을 맞춘 비사건 control과 비교한 코드입니다. 이후 Blox counterfactual replay에 사용할 Job Tape, Fault Tape, Risk Tape의 입력 적합성을 점검하고 5분 단위 OOF Risk Tape를 생성합니다.
...
- 30초 이내 반복 XID는 하나의 episode로 묶습니다.
- XID 43 기록 직전의 텔레메트리 하락에는 이미 GPU가 멈춘 뒤의 정보가 포함될 수 있습니다.
- 조기예측과 발생 후 incident 탐지를 구분하고, 시간순 OOF 분할로 미래 정보 유입을 막습니다.
- Blox replay에서는 과거 XID를 외생적 Fault Tape로 사용하고 Risk Tape는 GPU의 상대 위험 순위로 해석합니다.
- 원본 CSV와 대용량 중간 산출물은 `.gitignore`로 제외됩니다.
