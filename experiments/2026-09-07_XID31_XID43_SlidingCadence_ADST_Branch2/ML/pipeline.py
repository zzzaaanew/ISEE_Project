"""Unified Seren telemetry experiment for split, Sliding cadence comparison, and Branch-2-based ADST.
...
15-second source, and evaluates the integrated XID 31 OR XID 43 binary target
with a fixed chronological split, three Sliding retraining cadences, and ADST.
ADST keeps a six-hour retraining cadence and dynamically selects only the
historical Window from 7, 14, and 21 days using Branch 2 validation PR-AUC.

Calibration, multitask heads, GPU topology, Isolation Forest, Blox replay, and
separate result files are intentionally outside this run. Intermediate arrays
stay in memory; exactly two Excel workbooks are persistent outputs.
"""
...
SLIDING_RETRAIN_CADENCES_HOURS = (6, 24, 72)
ADST_RETRAIN_CADENCE_HOURS = 6
SLIDING_EXECUTION_TYPES = (
    "sliding_training_6h",
    "sliding_training_24h",
    "sliding_training_3d",
)
SLIDING_CADENCE_BY_MODE = {
    "sliding_training_6h": 6,
    "sliding_training_24h": 24,
    "sliding_training_3d": 72,
}
EXECUTION_TYPES = ("split", *SLIDING_EXECUTION_TYPES, "adst")
EXECUTION_TYPE = os.environ.get("EXECUTION_TYPE", "all").strip().lower()
if EXECUTION_TYPE not in ("all", "sliding_training", *EXECUTION_TYPES):
    raise ValueError(
        f"Unsupported EXECUTION_TYPE={EXECUTION_TYPE!r}; use all, split, "
        "sliding_training, sliding_training_6h, sliding_training_24h, "
        "sliding_training_3d, or adst."
    )
...
def cadence_label(hours: int) -> str:
    return "3 days" if int(hours) == 72 else f"{int(hours)} hours"


def make_dynamic_specs(
    ...
    cadence_bins = max(1, int(retrain_hours * 60 // 5))
    cadence_rows = max(1, int(cadence_bins // TEST_STRIDE_BINS))
    origin_rows = np.arange(0, len(test_times), cadence_rows, dtype=np.int32)
    window_days = (SLIDING_WINDOW_DAYS,) if mode in SLIDING_EXECUTION_TYPES else ADST_WINDOW_DAYS
    ...
    for origin_no, start_row in enumerate(origin_rows, start=1):
        ...
        for candidate_no, days in enumerate(window_days, start=1):
            ...
            train_sample, train_summary = sample_population(
                train_times, grid, ledger, n_gpu, rng, NEGATIVE_RATIO
            )
            validation_sample, validation_summary = sample_population(
                validation_times, grid, ledger, n_gpu, rng, None
            )
            specs.append({
                "origin_no": origin_no,
                "start_row": start_row,
                ...
                "retrain_cadence_hours": int(retrain_hours),
...
def fit_all_models(
    train_sample: dict[str, np.ndarray],
    validation_sample: dict[str, np.ndarray],
    parquet_root: Path,
    grid: np.ndarray,
    gpu_ids: list[str],
    std_maps: dict[str, dict[int, tuple[np.ndarray, np.ndarray]]],
    ledger: pd.DataFrame,
    seed: int,
    tag: str,
) -> dict[str, Any]:
    train_raw, train_context = materialize_samples(
        train_sample, parquet_root, grid, gpu_ids, std_maps, f"{tag} train"
    )
    validation_raw, validation_context = materialize_samples(
        validation_sample, parquet_root, grid, gpu_ids, std_maps, f"{tag} validation"
    )
    ...
    logistic = LogisticRegression(
        solver="lbfgs", class_weight="balanced", max_iter=250,
        random_state=seed, n_jobs=-1,
    )
    logistic.fit(flat_train, train_y)
    extra_trees = ExtraTreesClassifier(
        n_estimators=160, min_samples_leaf=5, max_features="sqrt",
        class_weight="balanced_subsample", n_jobs=-1, random_state=seed,
    )
    extra_trees.fit(flat_train, train_y)
    torch.manual_seed(seed + 1)
    cnn = train_network(
        f"{tag} 1D-CNN", OneDCNN(20), train_x, train_y,
        validation_x, validation_y, seed + 1,
    )
    torch.manual_seed(seed + 2)
    tcn = train_network(
        f"{tag} Tiny-TCN", TinyTCN(20), train_x, train_y,
        validation_x, validation_y, seed + 2,
    )
    branch1_models: dict[str, Any] = {
    ...
    train_context_x = make_context_matrix(
        train_raw, train_context, train_sample, grid, ledger
    )
    validation_context_x = make_context_matrix(
        validation_raw, validation_context, validation_sample, grid, ledger
    )
    context_mean, context_scale = fit_normalizer(train_context_x[:, None, :])
    context_train = np.where(
        np.isfinite(train_context_x), train_context_x, context_mean[None, :]
    ).astype(np.float32)
    context_validation = np.where(
        np.isfinite(validation_context_x),
        validation_context_x,
        context_mean[None, :],
    ).astype(np.float32)
    context_train = (context_train - context_mean[None, :]) / context_scale[None, :]
    context_validation = (
        context_validation - context_mean[None, :]
    ) / context_scale[None, :]
    context_lr = make_pipeline(
        StandardScaler(),
        LogisticRegression(
    ...
    context_gbdt = HistGradientBoostingClassifier(
        max_iter=180, learning_rate=0.08, max_leaf_nodes=31,
        l2_regularization=1.0, random_state=seed,
    )
    class_weights = np.where(
        train_y == 1,
        max(1.0, (train_y == 0).sum() / max(1, (train_y == 1).sum())),
        1.0,
    )
    context_gbdt.fit(context_train, train_y, sample_weight=class_weights)
    branch2_models = {
        "historical_logistic": context_lr,
        "historical_gbdt": context_gbdt,
    }
    validation_scores_b2 = {
        "historical_logistic": context_lr.predict_proba(context_validation)[:, 1],
        "historical_gbdt": context_gbdt.predict_proba(context_validation)[:, 1],
    }
    del (
        train_raw, validation_raw, train_x, validation_x, flat_train, flat_validation,
        context_train, context_validation, train_context_x, validation_context_x,
    )
    return {
...
def fill_test_scores(
    test_sample: dict[str, np.ndarray],
    parquet_root: Path,
    grid: np.ndarray,
    gpu_ids: list[str],
    std_maps: dict[str, dict[int, tuple[np.ndarray, np.ndarray]]],
    temporal_normalizer: tuple[np.ndarray, np.ndarray],
    temporal_models: dict[str, Any],
    context_models: dict[str, Any],
    context_train_stats: tuple[np.ndarray, np.ndarray],
    ledger: pd.DataFrame,
    batch_size: int = 2048,
    ...
    temporal_scores = {
        name: np.full((n_time, n_gpu), np.nan, dtype=np.float32)
    ...
    for completed, block in enumerate(unique_blocks, start=1):
        ...
        if len(local_t) == 0:
            continue
        sub = {
            "decision_index": local_times[local_t],
            "gpu_index": local_g.astype(np.int32),
            "y": labels[time_rows[local_t], local_g].astype(np.int8),
            "decision_times": local_times,
        }
        raw_x, raw_context = materialize_samples(
            sub, parquet_root, grid, gpu_ids, std_maps,
            f"test block {completed}/{len(unique_blocks)}",
        )
        ...
        ctx_x = make_context_matrix(
            raw_x, raw_context,
            {"decision_index": sub["decision_index"], "gpu_index": sub["gpu_index"]},
            grid, ledger,
        )
        ctx_x = np.where(
            np.isfinite(ctx_x), ctx_x, ctx_mean[None, :]
        ).astype(np.float32)
        ctx_x = (ctx_x - ctx_mean[None, :]) / ctx_scale[None, :]
        for name, model in context_models.items():
            context_scores[name][time_rows[local_t], local_g] = (
                model.predict_proba(ctx_x)[:, 1]
            )
        del raw_x, raw_context, norm_x, flat_x, ctx_x
    return temporal_scores, context_scores
...
def score_with_fit(
    ...
    temporal, context = fill_test_scores(
        sample,
        parquet_root,
        grid,
        gpu_ids,
        std_maps,
        fit["temporal_normalizer"],
        fit["temporal_models"],
        fit["context_models"],
        fit["context_stats"],
        ledger,
    )
    temporal["parallel_ensemble_rank_mean"] = parallel_rank_mean(
        temporal, sample["eligible_matrix"]
    )
    return temporal, context

def select_adst_window(
    ...
    for candidate in candidates:
        train_raw, train_context = materialize_samples(
            candidate["train_sample"], parquet_root, grid, gpu_ids, std_maps,
            f"adst origin {candidate['origin_no']} window {candidate['window_days']}d selector train",
        )
        validation_raw, validation_context = materialize_samples(
            candidate["validation_sample"], parquet_root, grid, gpu_ids, std_maps,
            f"adst origin {candidate['origin_no']} window {candidate['window_days']}d selector validation",
        )
        train_context_x = make_context_matrix(
            train_raw, train_context, candidate["train_sample"], grid, ledger
        )
        validation_context_x = make_context_matrix(
            validation_raw, validation_context, candidate["validation_sample"], grid, ledger
        )
        context_mean, context_scale = fit_normalizer(train_context_x[:, None, :])
        train_context_x = np.where(
            np.isfinite(train_context_x), train_context_x, context_mean[None, :]
        ).astype(np.float32)
        validation_context_x = np.where(
            np.isfinite(validation_context_x),
            validation_context_x,
            context_mean[None, :],
        ).astype(np.float32)
        train_context_x = (
            train_context_x - context_mean[None, :]
        ) / context_scale[None, :]
        validation_context_x = (
            validation_context_x - context_mean[None, :]
        ) / context_scale[None, :]
        selector = make_pipeline(
            StandardScaler(),
            LogisticRegression(
                solver="lbfgs", class_weight="balanced", max_iter=300,
                random_state=seed,
            ),
        )
        selector.fit(train_context_x, candidate["train_sample"]["y"])
        score = selector.predict_proba(validation_context_x)[:, 1]
        metric = flat_metrics(candidate["validation_sample"]["y"], score)
        scored.append({
            "window_days": candidate["window_days"],
            "selection_model": "branch2_historical_logistic",
            "selection_pr_auc": metric["pr_auc"],
            "selection_roc_auc": metric["roc_auc"],
            "selection_samples": metric["samples"],
            "selection_positive_samples": metric["positive_samples"],
        })
        del (
            train_raw, train_context, validation_raw, validation_context,
            train_context_x, validation_context_x,
        )
    best_index = int(np.nanargmax([
        row["selection_pr_auc"] if row["selection_pr_auc"] is not None else -np.inf
        for row in scored
    ]))
...
def run_dynamic_mode(
    ...
    for origin_no in sorted(grouped):
        ...
        if mode == "adst":
            selected, selector_rows = select_adst_window(
                candidates, parquet_root, grid, gpu_ids, std_maps,
                ledger, SEED + origin_no,
            )
        else:
            selected, selector_rows = candidates[0], []
        fit = fit_all_models(
            selected["train_sample"],
            selected["validation_sample"],
            parquet_root,
            grid,
            gpu_ids,
            std_maps,
        ...
        cadence_hours = int(selected["retrain_cadence_hours"])
        adaptation_rows.append({
            "execution_type": mode,
            "origin": origin_no,
            "test_start": utc_text(
                int(grid[test_sample["decision_times"][selected["start_row"]]] + STEP_NS)
            ),
            "test_end": utc_text(
                int(grid[test_sample["decision_times"][selected["end_row"] - 1]] + STEP_NS)
            ),
            "selected_window_days": selected["window_days"],
            "retrain_cadence": cadence_label(cadence_hours),
            "retrain_cadence_hours": cadence_hours,
            "candidate_validation": json.dumps(selector_rows, ensure_ascii=False),
        })
        segment = segment_test_sample(
            test_sample, selected["start_row"], selected["end_row"]
        )
        temporal, context = score_with_fit(
            segment, fit, parquet_root, grid, gpu_ids, std_maps, ledger
        )
        if not temporal_all:
            temporal_all = {
                name: np.full(
                    (len(test_sample["decision_times"]), len(gpu_ids)),
                    np.nan,
                    dtype=np.float32,
                )
        ...
        print(
            f"  {mode}: rolling origin {origin_no}/{len(grouped)} complete "
            f"(window={selected['window_days']}d, cadence={cadence_label(cadence_hours)})",
            flush=True,
        )
    return temporal_all, context_all, validation_rows, adaptation_rows
...
def main() -> None:
    ...
    if EXECUTION_TYPE == "all":
        selected_types = EXECUTION_TYPES
    elif EXECUTION_TYPE == "sliding_training":
        selected_types = SLIDING_EXECUTION_TYPES
    else:
        selected_types = (EXECUTION_TYPE,)
    ...
    validation_sample, validation_summary = sample_population(validation_times, grid, ledger, len(gpu_ids), rng, None)
    ...
    cadence_rows_by_mode: dict[str, int] = {}
    for mode in SLIDING_EXECUTION_TYPES:
        if mode in selected_types:
            cadence_hours = SLIDING_CADENCE_BY_MODE[mode]
            print(
                f"  preparing fixed {SLIDING_WINDOW_DAYS}-day Sliding origins "
                f"at {cadence_label(cadence_hours)} cadence...",
                flush=True,
            )
            dynamic_specs[mode], cadence_rows_by_mode[mode] = make_dynamic_specs(
                test_times, decision_indices, grid, ledger, len(gpu_ids),
                mode, cadence_hours,
            )
    if "adst" in selected_types:
        print("  preparing ADST candidate windows at 6-hour cadence...", flush=True)
        dynamic_specs["adst"], cadence_rows_by_mode["adst"] = make_dynamic_specs(
            test_times, decision_indices, grid, ledger, len(gpu_ids),
            "adst", ADST_RETRAIN_CADENCE_HOURS,
        )
    ...
    mode_outputs: dict[str, tuple[dict[str, np.ndarray], dict[str, np.ndarray]]] = {}
    validation_rows: list[dict[str, Any]] = []
    adaptation_rows: list[dict[str, Any]] = []
    if "split" in selected_types:
        print("[4/8] Training and scoring fixed chronological split...", flush=True)
        split_fit = fit_all_models(
            train_sample, validation_sample, parquet_root, grid, gpu_ids,
            std_maps, ledger, SEED, "split",
        )
        mode_outputs["split"] = score_with_fit(
            test_sample, split_fit, parquet_root, grid, gpu_ids, std_maps, ledger
        )
        for name, score in split_fit["validation_scores_b1"].items():
            validation_rows.append({
                "execution_type": "split",
                "evaluation_fold": "split_validation",
                "window_days": None,
                "model": name,
                **flat_metrics(validation_sample["y"], score),
            })
        ...
        adaptation_rows.append({
            "execution_type": "split",
            "origin": 0,
            "test_start": utc_text(int(grid[test_times[0]] + STEP_NS)),
            "test_end": utc_text(int(grid[test_times[-1]] + STEP_NS)),
            "selected_window_days": None,
            "retrain_cadence": "one fit",
            "retrain_cadence_hours": None,
            "candidate_validation": "",
        })

    for mode in (*SLIDING_EXECUTION_TYPES, "adst"):
        if mode not in selected_types:
            continue
        print(f"[5/8] Running {mode} rolling origins...", flush=True)
        temporal, context, dynamic_validation, dynamic_adaptation = run_dynamic_mode(
            mode, dynamic_specs[mode], test_sample, parquet_root, grid,
            gpu_ids, std_maps, ledger,
        )
        mode_outputs[mode] = (temporal, context)
    ...
    for mode, (temporal_scores, context_scores) in mode_outputs.items():
        b1_metrics.extend([
            {
                "execution_type": mode,
                "model": name,
                **evaluate_matrix(labels_test, eligible_test, scores),
            }
            for name, scores in temporal_scores.items()
        ])
        b2_metrics.extend([
            {
                "execution_type": mode,
            ...
            for name, scores in context_scores.items()
        ])
        b1_fold.extend([
            {"execution_type": mode, **row}
            for row in fold_rows(
                labels_test, eligible_test, temporal_scores,
                fold_metric_times, f"{mode}_test_temporal",
            )
        ])
        b2_fold.extend([
            {"execution_type": mode, **row}
            for row in fold_rows(
                labels_test, eligible_test, context_scores,
                fold_metric_times, f"{mode}_test_temporal",
            )
        ])
    ...
    for name, values in splits.items():
        split_summary.append({
            "execution_type": "split",
            "split": name,
            "available_grid_points": int(len(values)),
            "start": utc_text(int(grid[values.min()] + STEP_NS)),
            "end": utc_text(int(grid[values.max()] + STEP_NS)),
            "purge_between_splits": "36 hours",
            "window_days": None,
            "retrain_cadence": "one fit",
        })
    for mode in (*SLIDING_EXECUTION_TYPES, "adst"):
        if mode not in selected_types:
            continue
        if mode in SLIDING_CADENCE_BY_MODE:
            window_days: int | str = SLIDING_WINDOW_DAYS
            retrain_cadence = cadence_label(SLIDING_CADENCE_BY_MODE[mode])
        else:
            window_days = "7/14/21 candidate"
            retrain_cadence = cadence_label(ADST_RETRAIN_CADENCE_HOURS)
        split_summary.append({
            "execution_type": mode,
            "split": "test_rolling_origins",
            "available_grid_points": int(len(test_times)),
            "start": utc_text(int(grid[test_times.min()] + STEP_NS)),
            "end": utc_text(int(grid[test_times.max()] + STEP_NS)),
            "purge_between_splits": "36 hours",
    ...
    common_config = {
        ...
        "validation_sampling": "all eligible validation population; train negatives remain 4:1 sampled",
        ...
        "adst_window_selector": "Branch 2 historical logistic; validation PR-AUC",
        "adst_retraining": "fixed at 6 hours; only Window changes dynamically",
        "calibration": "not run by user decision",
        "multitask": "not run; integrated binary target only",
        "gpu_topology": "excluded; unavailable",
        "isolation_forest": "removed from this experiment",
        "random_seed": SEED,
        "persistent_outputs": "exactly two Excel workbooks; no CSV/Parquet/JSON/plot outputs",
    }
    config_rows: list[dict[str, Any]] = []
    for mode in selected_types:
        if mode == "split":
            training_policy = "fixed chronological split"
            retrain_policy = "one fit"
        elif mode in SLIDING_CADENCE_BY_MODE:
            cadence_hours = SLIDING_CADENCE_BY_MODE[mode]
            training_policy = (
                f"{SLIDING_WINDOW_DAYS}-day fixed Sliding window; "
                f"retrain every {cadence_label(cadence_hours)}"
            )
            retrain_policy = cadence_label(cadence_hours)
        else:
            training_policy = (
                "adaptive 7/14/21-day Window selected by Branch 2 "
                "validation PR-AUC; retrain every 6 hours"
            )
            retrain_policy = cadence_label(ADST_RETRAIN_CADENCE_HOURS)
        config_rows.append({
            **common_config,
            "execution_type": mode,
    ...
    base1 = common_sheets_multi(
        config_rows, audit_out, split_summary, sample_summary, event_summary
    )
    base2 = common_sheets_multi(
        config_rows, audit_out, split_summary, sample_summary, event_summary
    )
    adaptation_sheet = {
        "name": "adaptation_audit",
        "rows": rows_from_dicts(adaptation_rows),
    }
    payloads = [
        {
            "path": str(BRANCH_RESULT_FILES["Branch_1"]),
            "sheets": base1 + [
                adaptation_sheet,
                {"name": "model_metrics", "rows": rows_from_dicts(b1_metrics)},
                {"name": "fold_metrics", "rows": rows_from_dicts(b1_fold)},
                {"name": "validation_metrics", "rows": rows_from_dicts(validation_rows)},
            ],
        },
        {
            "path": str(BRANCH_RESULT_FILES["Branch_2"]),
            "sheets": base2 + [
                adaptation_sheet,
                {"name": "model_metrics", "rows": rows_from_dicts(b2_metrics)},
                {"name": "fold_metrics", "rows": rows_from_dicts(b2_fold)},
                {"name": "validation_metrics", "rows": rows_from_dicts(validation_rows)},
            ],
        },
    ]
    ...
    print(
        "[8/8] Writing exactly two verified Excel workbooks with all selected "
        "execution types...",
        flush=True,
    )
