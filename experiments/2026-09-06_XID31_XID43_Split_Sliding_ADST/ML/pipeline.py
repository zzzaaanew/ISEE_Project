"""Unified Seren telemetry experiment with split, sliding, and ADST modes.

This is the only executable ML file for the current experiment contract. It
reconstructs the target from the original 15-second Seren XID file, reads
the provided 5-minute Parquets, recomputes 5-minute standard deviations from
the 15-second source, and evaluates the integrated XID 31 OR XID 43 binary
target with three execution types: a fixed chronological split, fixed-window
sliding training, and adaptive dynamic sliding training (ADST). All execution
types are recorded in exactly three Excel workbooks.

Calibration, multitask heads, GPU topology, Blox replay, and separate result
files are intentionally outside this run. XID_TARGET_MODE remains available
for reproducing the earlier target-specific reference runs, but the official
experiment uses the default integrated target. Intermediate arrays stay in
memory; only the three requested Excel workbooks are persistent outputs.
"""
...
EXECUTION_TYPES = ("split", "sliding_training", "adst")
EXECUTION_TYPE = os.environ.get("EXECUTION_TYPE", "all").strip().lower()
if EXECUTION_TYPE not in ("all", *EXECUTION_TYPES):
    raise ValueError(f"Unsupported EXECUTION_TYPE={EXECUTION_TYPE!r}; use all, split, sliding_training, or adst.")
SLIDING_WINDOW_DAYS = 14
ADST_WINDOW_DAYS = (7, 14, 21)
ADST_HALF_LIFE_DAYS = 3
RETRAIN_CADENCE_BINS = 72  # six hours on the five-minute grid
...
FEATURE_BLOCK_CACHE: dict[tuple[int, tuple[str, ...]], tuple[np.ndarray, np.ndarray]] = {}
TELEMETRY_FILES = {
...
def materialize_samples(sample: dict[str, np.ndarray], parquet_root: Path, grid: np.ndarray, gpu_ids: list[str], std_maps: dict[str, dict[int, tuple[np.ndarray, np.ndarray]]], title: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    for completed, block in enumerate(unique_blocks, start=1):
        ...
        block_start = max(0, int(block) * 144 - HISTORY_START_OFFSET - 1)
        block_end = min(len(grid) - 1, (int(block) + 1) * 144 - FEATURE_GAP_BINS)
        cache_key = (int(block), tuple(gpu_ids))
        cached = FEATURE_BLOCK_CACHE.get(cache_key)
        if cached is None:
            cached = load_feature_block(parquet_root, grid, block_start, block_end, gpu_ids, std_maps)
            FEATURE_BLOCK_CACHE[cache_key] = cached
        temporal, context = cached
        local = decisions[:, None] - HISTORY_START_OFFSET - block_start + np.arange(HISTORY_BINS)[None, :]
...
def train_network(name: str, model: nn.Module, x_train: np.ndarray, y_train: np.ndarray, x_validation: np.ndarray, y_validation: np.ndarray, seed: int, epochs: int = 5, batch_size: int = 2048, sample_weights: np.ndarray | None = None) -> nn.Module:
    rng = np.random.default_rng(seed)
    positive = max(1, int(y_train.sum()))
    negative = max(1, len(y_train) - positive)
    loss_fn = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(negative / positive, dtype=torch.float32), reduction="none" if sample_weights is not None else "mean")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for start in range(0, len(order), batch_size):
            ...
            if sample_weights is not None:
                batch_weights = torch.from_numpy(sample_weights[rows].astype(np.float32, copy=False))
...
def sampled_window_times(decision_indices: np.ndarray, cutoff_index: int, window_days: int) -> tuple[np.ndarray, np.ndarray]:
    window_bins = int(window_days * 24 * 60 * 60 * 1_000_000_000 // STEP_NS)
    available = decision_indices[(decision_indices < cutoff_index) & (decision_indices >= cutoff_index - window_bins)]
    sampled = available[::TEST_STRIDE_BINS]
    if len(sampled) < 20:
        sampled = available
    if len(sampled) < 10:
        raise ValueError(f"Not enough matured decision times for a {window_days}-day sliding window.")
    split_at = max(1, min(len(sampled) - 1, int(len(sampled) * 0.8)))
    return sampled[:split_at].astype(np.int32), sampled[split_at:].astype(np.int32)


def make_dynamic_specs(test_times: np.ndarray, decision_indices: np.ndarray, grid: np.ndarray, ledger: pd.DataFrame, n_gpu: int, mode: str) -> tuple[list[dict[str, Any]], int]:
    cadence_rows = max(1, int(RETRAIN_CADENCE_BINS // TEST_STRIDE_BINS))
    origin_rows = np.arange(0, len(test_times), cadence_rows, dtype=np.int32)
    window_days = (SLIDING_WINDOW_DAYS,) if mode == "sliding_training" else ADST_WINDOW_DAYS
    specs: list[dict[str, Any]] = []
    for origin_no, start_row in enumerate(origin_rows, start=1):
        start_row = int(start_row)
        end_row = min(start_row + cadence_rows, len(test_times))
        cutoff_index = int(test_times[start_row] - PURGE_NS // STEP_NS)
        for candidate_no, days in enumerate(window_days, start=1):
            train_times, validation_times = sampled_window_times(decision_indices, cutoff_index, int(days))
            rng = np.random.default_rng(SEED + origin_no * 100 + candidate_no)
            train_sample, train_summary = sample_population(train_times, grid, ledger, n_gpu, rng, NEGATIVE_RATIO)
            validation_sample, validation_summary = sample_population(validation_times, grid, ledger, n_gpu, rng, NEGATIVE_RATIO)
            specs.append({
                "origin_no": origin_no,
                "start_row": start_row,
                "end_row": end_row,
    ...
    return specs, cadence_rows


def recency_weights(sample: dict[str, np.ndarray], half_life_days: int) -> np.ndarray:
    latest = int(sample["decision_index"].max())
    age_days = (latest - sample["decision_index"].astype(np.float64)) * STEP_NS / 86_400_000_000_000.0
    weights = np.exp(-np.log(2.0) * age_days / max(float(half_life_days), 1.0)).astype(np.float32)
    return (weights / max(float(weights.mean()), 1e-6)).astype(np.float32)


def fit_all_models(train_sample: dict[str, np.ndarray], validation_sample: dict[str, np.ndarray], parquet_root: Path, grid: np.ndarray, gpu_ids: list[str], std_maps: dict[str, dict[int, tuple[np.ndarray, np.ndarray]]], ledger: pd.DataFrame, seed: int, tag: str, sample_weights: np.ndarray | None = None) -> dict[str, Any]:
    train_raw, train_context = materialize_samples(train_sample, parquet_root, grid, gpu_ids, std_maps, f"{tag} train")
    validation_raw, validation_context = materialize_samples(validation_sample, parquet_root, grid, gpu_ids, std_maps, f"{tag} validation")
    ...
    fit_kwargs = {} if sample_weights is None else {"sample_weight": sample_weights}
    logistic = LogisticRegression(solver="lbfgs", class_weight="balanced", max_iter=250, random_state=seed, n_jobs=-1)
    logistic.fit(flat_train, train_y, **fit_kwargs)
    ...
    extra_trees.fit(flat_train, train_y, **fit_kwargs)
    torch.manual_seed(seed + 1)
    cnn = train_network(f"{tag} 1D-CNN", OneDCNN(20), train_x, train_y, validation_x, validation_y, seed + 1, sample_weights=sample_weights)
    torch.manual_seed(seed + 2)
    tcn = train_network(f"{tag} Tiny-TCN", TinyTCN(20), train_x, train_y, validation_x, validation_y, seed + 2, sample_weights=sample_weights)
    branch1_models: dict[str, Any] = {"logistic_regression": logistic, "extra_trees": extra_trees, "one_d_cnn": cnn, "tiny_tcn": tcn}
    validation_scores_b1 = {
        "logistic_regression": logistic.predict_proba(flat_validation)[:, 1],
        "extra_trees": extra_trees.predict_proba(flat_validation)[:, 1],
        "one_d_cnn": network_probabilities(cnn, validation_x),
        "tiny_tcn": network_probabilities(tcn, validation_x),
    }
    ...
    context_fit_kwargs = {} if sample_weights is None else {"logisticregression__sample_weight": sample_weights}
    context_lr.fit(context_train, train_y, **context_fit_kwargs)
    ...
    class_weights = np.where(train_y == 1, max(1.0, (train_y == 0).sum() / max(1, (train_y == 1).sum())), 1.0)
    gbdt_weights = class_weights if sample_weights is None else class_weights * sample_weights
    context_gbdt.fit(context_train, train_y, sample_weight=gbdt_weights)
    ...
    validation_score_b3 = isolation_forest.score_samples(observability_matrix(validation_raw)) * -1.0
    del train_raw, validation_raw, train_x, validation_x, flat_train, flat_validation, context_train, context_validation, train_context_x, validation_context_x, train_obs
    return {
        "temporal_models": branch1_models,
        "context_models": branch2_models,
        "observation_models": branch3_models,
        "temporal_normalizer": (temporal_mean, temporal_scale),
        "context_stats": (context_mean, context_scale),
        "validation_scores_b1": validation_scores_b1,
        "validation_scores_b2": validation_scores_b2,
        "validation_score_b3": validation_score_b3,
    }
...
def segment_test_sample(test_sample: dict[str, np.ndarray], start_row: int, end_row: int) -> dict[str, np.ndarray]:
    rows = np.arange(start_row, end_row, dtype=np.int32)
    decision_times = test_sample["decision_times"][rows]
    labels = test_sample["labels_matrix"][rows]
    eligible = test_sample["eligible_matrix"][rows]
    local_t, local_g = np.where(eligible)
    return {
        "decision_index": decision_times[local_t].astype(np.int32),
        "gpu_index": local_g.astype(np.int32),
        "y": labels[local_t, local_g].astype(np.int8),
...
def score_with_fit(sample: dict[str, np.ndarray], fit: dict[str, Any], parquet_root: Path, grid: np.ndarray, gpu_ids: list[str], std_maps: dict[str, dict[int, tuple[np.ndarray, np.ndarray]]], ledger: pd.DataFrame) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray], dict[str, np.ndarray]]:
    temporal, context, observation = fill_test_scores(sample, parquet_root, grid, gpu_ids, std_maps, fit["temporal_normalizer"], {**fit["temporal_models"], **fit["observation_models"]}, fit["context_models"], fit["context_stats"], ledger)
    temporal["parallel_ensemble_rank_mean"] = parallel_rank_mean(temporal, sample["eligible_matrix"])
    return temporal, context, observation


def select_adst_window(candidates: list[dict[str, Any]], parquet_root: Path, grid: np.ndarray, gpu_ids: list[str], std_maps: dict[str, dict[int, tuple[np.ndarray, np.ndarray]]], ledger: pd.DataFrame, seed: int) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    scored: list[dict[str, Any]] = []
    for candidate in candidates:
        train_raw, _ = materialize_samples(candidate["train_sample"], parquet_root, grid, gpu_ids, std_maps, f"adst origin {candidate['origin_no']} window {candidate['window_days']}d selector train")
        validation_raw, _ = materialize_samples(candidate["validation_sample"], parquet_root, grid, gpu_ids, std_maps, f"adst origin {candidate['origin_no']} window {candidate['window_days']}d selector validation")
        mean, scale = fit_normalizer(train_raw)
        train_x = normalize(train_raw, mean, scale).reshape(len(train_raw), -1)
        validation_x = normalize(validation_raw, mean, scale).reshape(len(validation_raw), -1)
        selector = LogisticRegression(solver="lbfgs", class_weight="balanced", max_iter=250, random_state=seed, n_jobs=-1)
        selector.fit(train_x, candidate["train_sample"]["y"])
        score = selector.predict_proba(validation_x)[:, 1]
        metric = flat_metrics(candidate["validation_sample"]["y"], score)
        scored.append({"window_days": candidate["window_days"], "selection_pr_auc": metric["pr_auc"], "selection_roc_auc": metric["roc_auc"]})
        del train_raw, validation_raw, train_x, validation_x
    best_index = int(np.nanargmax([row["selection_pr_auc"] for row in scored]))
    best = candidates[best_index]
    return best, scored


def run_dynamic_mode(mode: str, specs: list[dict[str, Any]], test_sample: dict[str, np.ndarray], parquet_root: Path, grid: np.ndarray, gpu_ids: list[str], std_maps: dict[str, dict[int, tuple[np.ndarray, np.ndarray]]], ledger: pd.DataFrame, cadence_rows: int) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray], dict[str, np.ndarray], list[dict[str, Any]], list[dict[str, Any]]]:
    grouped: dict[int, list[dict[str, Any]]] = defaultdict(list)
    for spec in specs:
        grouped[int(spec["origin_no"])].append(spec)
    temporal_all: dict[str, np.ndarray] = {}
    context_all: dict[str, np.ndarray] = {}
    observation_all: dict[str, np.ndarray] = {}
    validation_rows: list[dict[str, Any]] = []
    adaptation_rows: list[dict[str, Any]] = []
    for origin_no in sorted(grouped):
        candidates = sorted(grouped[origin_no], key=lambda item: item["window_days"])
        if mode == "adst":
            selected, selector_rows = select_adst_window(candidates, parquet_root, grid, gpu_ids, std_maps, ledger, SEED + origin_no)
        else:
            selected, selector_rows = candidates[0], []
        weights = recency_weights(selected["train_sample"], ADST_HALF_LIFE_DAYS) if mode == "adst" else None
        fit = fit_all_models(selected["train_sample"], selected["validation_sample"], parquet_root, grid, gpu_ids, std_maps, ledger, SEED + origin_no, f"{mode} origin {origin_no}", weights)
        validation_y = selected["validation_sample"]["y"]
        for name, score in fit["validation_scores_b1"].items():
            validation_rows.append({"execution_type": mode, "evaluation_fold": f"origin_{origin_no}", "window_days": selected["window_days"], "model": name, **flat_metrics(validation_y, score)})
        for name, score in fit["validation_scores_b2"].items():
            validation_rows.append({"execution_type": mode, "evaluation_fold": f"origin_{origin_no}", "window_days": selected["window_days"], "model": name, **flat_metrics(validation_y, score)})
        validation_rows.append({"execution_type": mode, "evaluation_fold": f"origin_{origin_no}", "window_days": selected["window_days"], "model": "isolation_forest", **flat_metrics(validation_y, fit["validation_score_b3"])})
        adaptation_rows.append({
            "execution_type": mode,
            "origin": origin_no,
            "test_start": utc_text(int(grid[test_sample["decision_times"][selected["start_row"]]] + STEP_NS)),
            "test_end": utc_text(int(grid[test_sample["decision_times"][selected["end_row"] - 1]] + STEP_NS)),
            "selected_window_days": selected["window_days"],
            "retrain_cadence": "6 hours",
            "candidate_validation": json.dumps(selector_rows, ensure_ascii=False),
        })
        segment = segment_test_sample(test_sample, selected["start_row"], selected["end_row"])
        temporal, context, observation = score_with_fit(segment, fit, parquet_root, grid, gpu_ids, std_maps, ledger)
        if not temporal_all:
            temporal_all = {name: np.full((len(test_sample["decision_times"]), len(gpu_ids)), np.nan, dtype=np.float32) for name in temporal}
            context_all = {name: np.full((len(test_sample["decision_times"]), len(gpu_ids)), np.nan, dtype=np.float32) for name in context}
            observation_all = {name: np.full((len(test_sample["decision_times"]), len(gpu_ids)), np.nan, dtype=np.float32) for name in observation}
        for name, scores in temporal.items():
            temporal_all[name][selected["start_row"] : selected["end_row"]] = scores
        for name, scores in context.items():
            context_all[name][selected["start_row"] : selected["end_row"]] = scores
        for name, scores in observation.items():
            observation_all[name][selected["start_row"] : selected["end_row"]] = scores
        print(f"  {mode}: rolling origin {origin_no}/{len(grouped)} complete (window={selected['window_days']}d)", flush=True)
    return temporal_all, context_all, observation_all, validation_rows, adaptation_rows
...
def common_sheets_multi(config_rows: list[dict[str, Any]], audit: dict[str, Any], split_summary: list[dict[str, Any]], sample_summary: list[dict[str, Any]], event_summary: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {"name": "run_config", "rows": rows_from_dicts(config_rows)},
        {"name": "data_audit", "rows": rows_from_dicts([audit])},
        {"name": "time_split", "rows": rows_from_dicts(split_summary)},
        {"name": "sample_audit", "rows": rows_from_dicts(sample_summary)},
        {"name": "event_summary", "rows": rows_from_dicts(event_summary)},
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError("Required inputs are missing: " + ", ".join(missing))
    selected_types = EXECUTION_TYPES if EXECUTION_TYPE == "all" else (EXECUTION_TYPE,)

    print(f"[1/8] Reconstructing integrated target for {TARGET_MODE} from raw Seren...", flush=True)
    gpu_ids = read_gpu_ids(xid_path)
    ledger, audit = reconstruct_target_episodes(xid_path, gpu_ids, TARGET_CODES)
    target_text = " OR ".join(f"XID {code}" for code in TARGET_CODES)
    audit["target_definition"] = f"integrated binary: future {target_text} onset within 24 hours"
    audit["target_codes"] = ",".join(str(code) for code in TARGET_CODES)
    audit["gpu_count"] = len(gpu_ids)
    print(f"  target episodes={len(ledger):,}, certain={audit['certain_episode_count']:,}, uncertain={audit['uncertain_episode_count']:,}", flush=True)
    ...
    cadence_rows = max(1, int(RETRAIN_CADENCE_BINS // TEST_STRIDE_BINS))
    if "sliding_training" in selected_types:
        print("  preparing fixed-window sliding origins...", flush=True)
        dynamic_specs["sliding_training"], cadence_rows = make_dynamic_specs(test_times, decision_indices, grid, ledger, len(gpu_ids), "sliding_training")
    if "adst" in selected_types:
        print("  preparing ADST candidate windows...", flush=True)
        dynamic_specs["adst"], cadence_rows = make_dynamic_specs(test_times, decision_indices, grid, ledger, len(gpu_ids), "adst")
    dynamic_samples = [sample for specs in dynamic_specs.values() for spec in specs for sample in (spec["train_sample"], spec["validation_sample"])]

    def summary_row(execution_type: str, population: str, summary: dict[str, Any], fold: str | None = None, window_days: int | None = None) -> dict[str, Any]:
        return {"execution_type": execution_type, "population": population, "fold": fold, "window_days": window_days, **summary}

    sample_summary = []
    if "split" in selected_types:
        sample_summary.extend([summary_row("split", "train", train_summary), summary_row("split", "validation", validation_summary), summary_row("split", "test", test_summary)])
    for mode, specs in dynamic_specs.items():
        for spec in specs:
            fold = f"origin_{spec['origin_no']}"
            sample_summary.append(summary_row(mode, "train", spec["train_summary"], fold, spec["window_days"]))
            sample_summary.append(summary_row(mode, "validation", spec["validation_summary"], fold, spec["window_days"]))
        sample_summary.append(summary_row(mode, "test", test_summary))

    print("[3/8] Recomputing requested 5-minute standard deviations from raw 15-second metrics...", flush=True)
    needed_buckets = build_needed_buckets([train_sample, validation_sample, test_sample, *dynamic_samples])
    std_maps = recompute_raw_std(raw_dir, grid, needed_buckets)
    print(f"  requested bucket/GPU groups={sum(len(v) for v in needed_buckets.values()):,}", flush=True)

    mode_outputs: dict[str, tuple[dict[str, np.ndarray], dict[str, np.ndarray], dict[str, np.ndarray]]] = {}
    validation_rows: list[dict[str, Any]] = []
    adaptation_rows: list[dict[str, Any]] = []
    if "split" in selected_types:
        print("[4/8] Training and scoring fixed chronological split...", flush=True)
        split_fit = fit_all_models(train_sample, validation_sample, parquet_root, grid, gpu_ids, std_maps, ledger, SEED, "split")
        mode_outputs["split"] = score_with_fit(test_sample, split_fit, parquet_root, grid, gpu_ids, std_maps, ledger)
        for name, score in split_fit["validation_scores_b1"].items():
            validation_rows.append({"execution_type": "split", "evaluation_fold": "split_validation", "window_days": None, "model": name, **flat_metrics(validation_sample["y"], score)})
        for name, score in split_fit["validation_scores_b2"].items():
            validation_rows.append({"execution_type": "split", "evaluation_fold": "split_validation", "window_days": None, "model": name, **flat_metrics(validation_sample["y"], score)})
        validation_rows.append({"execution_type": "split", "evaluation_fold": "split_validation", "window_days": None, "model": "isolation_forest", **flat_metrics(validation_sample["y"], split_fit["validation_score_b3"])})
        adaptation_rows.append({"execution_type": "split", "origin": 0, "test_start": utc_text(int(grid[test_times[0]] + STEP_NS)), "test_end": utc_text(int(grid[test_times[-1]] + STEP_NS)), "selected_window_days": None, "retrain_cadence": "one fit", "candidate_validation": ""})

    for mode in ("sliding_training", "adst"):
        if mode not in selected_types:
            continue
        print(f"[5/8] Running {mode} rolling origins...", flush=True)
        dynamic_result = run_dynamic_mode(mode, dynamic_specs[mode], test_sample, parquet_root, grid, gpu_ids, std_maps, ledger, cadence_rows)
        mode_outputs[mode] = dynamic_result[:3]
        dynamic_validation, dynamic_adaptation = dynamic_result[3:]
        validation_rows.extend(dynamic_validation)
        adaptation_rows.extend(dynamic_adaptation)

    labels_test, eligible_test = test_sample["labels_matrix"], test_sample["eligible_matrix"]
    b1_metrics: list[dict[str, Any]] = []
    b2_metrics: list[dict[str, Any]] = []
    b3_metrics: list[dict[str, Any]] = []
    b1_fold: list[dict[str, Any]] = []
    b2_fold: list[dict[str, Any]] = []
    b3_fold: list[dict[str, Any]] = []
    fold_metric_times = grid[test_sample["decision_times"]] + STEP_NS
    for mode, (temporal_scores, context_scores, observation_scores) in mode_outputs.items():
        b1_metrics.extend([{ "execution_type": mode, "model": name, **evaluate_matrix(labels_test, eligible_test, scores)} for name, scores in temporal_scores.items()])
        b2_metrics.extend([{ "execution_type": mode, "model": name, **evaluate_matrix(labels_test, eligible_test, scores)} for name, scores in context_scores.items()])
        b3_metrics.extend([{ "execution_type": mode, "model": name, **evaluate_matrix(labels_test, eligible_test, scores)} for name, scores in observation_scores.items()])
        b1_fold.extend([{ "execution_type": mode, **row} for row in fold_rows(labels_test, eligible_test, temporal_scores, fold_metric_times, f"{mode}_test_temporal")])
        b2_fold.extend([{ "execution_type": mode, **row} for row in fold_rows(labels_test, eligible_test, context_scores, fold_metric_times, f"{mode}_test_temporal")])
        b3_fold.extend([{ "execution_type": mode, **row} for row in fold_rows(labels_test, eligible_test, observation_scores, fold_metric_times, f"{mode}_test_temporal")])

    event_summary = [{"xid_code": int(code), "episodes": int((ledger["xid_code"] == code).sum()), "certain_episodes": int(((ledger["xid_code"] == code) & (~ledger["uncertain_onset"])).sum()), "uncertain_episodes": int(((ledger["xid_code"] == code) & ledger["uncertain_onset"]).sum())} for code in TARGET_CODES]
    split_summary = []
    for name, values in splits.items():
        split_summary.append({"execution_type": "split", "split": name, "available_grid_points": int(len(values)), "start": utc_text(int(grid[values.min()] + STEP_NS)), "end": utc_text(int(grid[values.max()] + STEP_NS)), "purge_between_splits": "36 hours", "window_days": None, "retrain_cadence": "one fit"})
    for mode in ("sliding_training", "adst"):
        if mode in selected_types:
            split_summary.append({"execution_type": mode, "split": "test_rolling_origins", "available_grid_points": int(len(test_times)), "start": utc_text(int(grid[test_times.min()] + STEP_NS)), "end": utc_text(int(grid[test_times.max()] + STEP_NS)), "purge_between_splits": "36 hours", "window_days": SLIDING_WINDOW_DAYS if mode == "sliding_training" else "7/14/21 candidate", "retrain_cadence": "6 hours"})

    common_config = {
        "target": f"integrated binary {TARGET_MODE} ({'/'.join(f'XID{code}' for code in TARGET_CODES)})",
        "horizon": "24 hours",
        "input_window": "1 hour = 12 x 5-minute buckets",
        "feature_cutoff": "last 10 minutes excluded; history buckets decision-65m through decision-10m",
        "telemetry_source": "Seren only; 5-minute Parquet mean/obs plus raw 15-second recomputed std",
        ...
        "branch_3_model": "Isolation Forest on observability/missingness only",
        "calibration": "not run by user decision",
        "multitask": "not run; integrated binary target only",
        "gpu_topology": "excluded; unavailable",
        "random_seed": SEED,
        "persistent_outputs": "exactly three Excel workbooks; no CSV/Parquet/JSON/plot outputs",
    }
    config_rows = []
    for mode in selected_types:
        config_rows.append({**common_config, "execution_type": mode, "training_policy": "fixed chronological split" if mode == "split" else ("14-day fixed sliding window" if mode == "sliding_training" else "adaptive 7/14/21-day window with 3-day half-life recency weights"), "retrain_policy": "one fit" if mode == "split" else "every 6 hours"})
    audit_out = {**audit, "parquet_root": str(parquet_root), "raw_seren_directory": str(raw_dir), "parquet_grid_points": int(len(grid)), "parquet_gpu_count": int(len(gpu_ids)), "recomputed_std_metric_count": 4, "std_requested_bucket_count": int(len(needed_buckets))}
    base1 = common_sheets_multi(config_rows, audit_out, split_summary, sample_summary, event_summary)
    base2 = common_sheets_multi(config_rows, audit_out, split_summary, sample_summary, event_summary)
    base3 = common_sheets_multi(config_rows, audit_out, split_summary, sample_summary, event_summary)
    adaptation_sheet = {"name": "adaptation_audit", "rows": rows_from_dicts(adaptation_rows)}
    payloads = [
        {"path": str(BRANCH_RESULT_FILES["Branch_1"]), "sheets": base1 + [adaptation_sheet, {"name": "model_metrics", "rows": rows_from_dicts(b1_metrics)}, {"name": "fold_metrics", "rows": rows_from_dicts(b1_fold)}, {"name": "validation_metrics", "rows": rows_from_dicts(validation_rows)}]},
        {"path": str(BRANCH_RESULT_FILES["Branch_2"]), "sheets": base2 + [adaptation_sheet, {"name": "model_metrics", "rows": rows_from_dicts(b2_metrics)}, {"name": "fold_metrics", "rows": rows_from_dicts(b2_fold)}, {"name": "validation_metrics", "rows": rows_from_dicts(validation_rows)}]},
        {"path": str(BRANCH_RESULT_FILES["Branch_3_optional"]), "sheets": base3 + [adaptation_sheet, {"name": "model_metrics", "rows": rows_from_dicts(b3_metrics)}, {"name": "fold_metrics", "rows": rows_from_dicts(b3_fold)}, {"name": "validation_metrics", "rows": rows_from_dicts(validation_rows)}]},
    ]
    for path in BRANCH_RESULT_FILES.values():
        path.parent.mkdir(parents=True, exist_ok=True)
    print("[8/8] Writing exactly three Excel workbooks with all selected execution types...", flush=True)
    write_workbooks(payloads)
    for path in BRANCH_RESULT_FILES.values():
        print(f"  wrote {path}", flush=True)
